...
class TestSACModel:
    def test_init(self):
        ...
        assert isinstance(sac.feature_extractor, MLP), (
            "Feature extractor should be Sequential"
        )
        assert len(sac.feature_extractor.layers) == 4, (
            "Feature extractor should have 2 layers + 2 activation layers"
        )
        assert sac.feature_extractor.layers[0].in_features == 8, (
            "First layer should take obs_size as input"
        )
        assert isinstance(sac.feature_extractor.layers[1], nn.ReLU), (
            "Feature extractor should have ReLU activation"
        )
        assert sac.feature_extractor.layers[-2].out_features == 256, (
            "Last layer should output 256 features"
        )
        assert isinstance(sac.policy_net, nn.Linear), "Policy network should be Linear"
        assert sac.policy_net.out_features == 6, (
            "Last layer should output action_size*2"
        )

        assert isinstance(sac.q_net1, nn.Sequential), "Q-network 1 should be Sequential"
        assert len(sac.q_net1) == 5, (
            "Q-network 1 should have 3 layers + 2 activation layers"
        )
        assert sac.q_net1[0].in_features == 11, (
            "Q-network should take obs_size + action_size as input"
        )
        assert sac.q_net1[0].out_features == 256, (
            "Q-network first layer should output 256 features"
        )
        assert sac.q_net1[-1].out_features == 1, (
            "Q-network should output a single value"
        )

        assert isinstance(sac.q_net2, nn.Sequential), "Q-network 2 should be Sequential"
        assert len(sac.q_net2) == 5, (
            "Q-network 2 should have 3 layers + 2 activation layers"
        )
        assert isinstance(sac.target_q_net1, nn.Sequential), (
            "Target Q-network 1 should be Sequential"
        )
        assert len(sac.target_q_net1) == 5, (
            "Target Q-network 1 should have 3 layers + 2 activation layers"
        )
        assert isinstance(sac.target_q_net2, nn.Sequential), (
            "Target Q-network 2 should be Sequential"
        )
        assert len(sac.target_q_net2) == 5, (
            "Target Q-network 2 should have 3 layers + 2 activation layers"
        )
    ...
    def test_init_custom_params(self):
        ...
        assert sac.feature_extractor.layers[0].in_features == 4, (
            "Feature extractor should take obs_size as input"
        )
        assert sac.feature_extractor.layers[0].out_features == 128, (
            "Feature extractor first layer should output 128 features"
        )
        assert isinstance(sac.feature_extractor.layers[1], nn.Tanh), (
            "Feature extractor should have tanh activation"
        )
        assert sac.feature_extractor.layers[2].out_features == 64, (
            "Feature extractor second layer should output 64 features"
        )

        assert len(sac.q_net1) == 3, (
            "Q-network 1 should have 2 layers + 1 activation layer"
        )
        assert sac.q_net1[0].in_features == 6, (
            "Q-network should take obs_size + action_size as input"
        )
        assert sac.q_net1[0].out_features == 64, (
            "Q-network first layer should output 64 features"
        )
        assert sac.q_net1[-1].out_features == 1, (
            "Q-network should output a single value"
        )
    ...
    def test_twin_q_networks_independence(self):
        ...
        assert params_different, "Q-networks should have different parameters"
    ...
    def test_load_state_dict(self):
        ...
        with torch.no_grad():
            _, _, mean2_before, _ = sac2(dummy_state, deterministic=True)
            q1_2_before = sac2.forward_q1(state_action)
            sac2.forward_q2(state_action)
        ...
        with torch.no_grad():
            _, _, mean2_after, log_std2_after = sac2(
    ...
    def test_numerical_stability(self):
        ...
        boundary_log_prob = sample_nondeterministic_logprobs(
            boundary_z, boundary_mean, boundary_log_std, sac=True
        )
