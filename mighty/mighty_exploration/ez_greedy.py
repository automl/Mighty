...
class EZGreedy(BaseValueBasedPolicy):
    r"""
    ...
    """
    def __init__(self, q, epsilon=0.1, skip=100):
        super().__init__(q)
    ...
    def __call__(self, s, return_logp=False):
        if self.skipped >= self.skip or self.current_params is None:
            ...
            self.action = self.proba_dist.postprocess_variate(self.rng, X)
            self.logprobs = batch_to_single(logP)
            self.skipped = 0
        else:
            self.skipped += 1
    ...
    @property
    def params(self):
        return hk.data_structures.to_immutable_dict({'epsilon': self.epsilon, 'q': self.q.params, 'skip': self.skip})
